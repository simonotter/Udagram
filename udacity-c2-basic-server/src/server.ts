...
(async () => {
  ...
  app.get( "/cars/:id",
    ...
    // respond not found, if we don't have this id
    if ( car && car.length === 0 ) {
      return res.status(404)
                .send('car with id ' + id + ' cannot be found');
    }
  ...
  app.post( "/cars",
    ( req: Request, res: Response ) => {
      ...
      if ( !make || !type || !model || !cost ) {
        // respond with error if not
        return res.status(400)
                  .send(`make, type, model and cost are required`);
      }
      ...
      // add this car to our local array of cars
      cars.push(new_car);

      // send the complete car object as a response
      // along with a 201 status - creation success
